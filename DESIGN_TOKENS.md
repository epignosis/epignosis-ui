# Epignosis UI Design Tokens

Shared Epignosis design tokens, aligned with Figma. These tokens are **framework-agnostic** and can be used in any project — React, Vue, Svelte, Angular, or vanilla JS/CSS.

## Installation

Installed directly from the repo (not published to npm):
...
```js
const { colors, spacing } = require("epignosis-ui/tokens");
```
...
<template>
  <button :style="{ backgroundColor: colors.primary.base, padding: spacing.md }">Click me</button>
</template>
```
...
```html
<link rel="stylesheet" href="node_modules/epignosis-ui/src/theme/tokens.css" />
